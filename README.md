...
Every image has a stable `imageId`, retained when moving or replacing it. Ordinary page drafts still use `editor-data/pages/<pageId>.json` on the existing `documentation-drafts` branch, but contain no `scriptNote` values. Notes live separately in `editor-data/image-notes/<pageId>.json`, keyed by `imageId`; English uses `editor-data/en/` for both paths. Opening a page merges those notes into its image nodes for the unchanged note UI. Draft and note updates share one Git commit with non-forced branch updates and revision checks. Local recovery also uses separate IndexedDB stores. Legacy inline notes migrate on the next save or draft deletion, and legacy image IDs match across base64 and published file URLs.

«Настройки страницы» → «Удалить черновик» removes the current locale's draft after confirmation. The editor reads the latest published version from a pinned GitHub revision, restores its title/content and reattaches saved notes by image ID, including note edits not yet autosaved. Notes belonging only to unpublished images are removed. Notes for published images survive even if those images were removed or replaced in the discarded draft. The recovery draft is cleared; a deletion timestamp prevents older cached drafts from reopening automatically. Existing Git revision history remains available. A page without a published version in the selected language cannot be reset this way.
