...
- PlantUML: исходный код и рендер рядом в редакторе, только PNG в публикации и HTML-экспорте; рендер обновляется кнопкой через публичный сервис Kroki (код отправляется сервису), изображение сохраняется вместе с черновиком;
- карусели из нескольких изображений/GIF: автопрокрутка каждые 3 секунды, стрелки, точки, пауза, изменение порядка и описаний; до 12 файлов по 5 МБ, суммарно до 15 МБ;
- таблицы с добавлением, удалением, объединением и разделением строк/столбцов, изменением ширины столбцов;
- независимое закрепление первой строки и первого столбца в прокручиваемой области таблицы, в том числе в опубликованной документации;
