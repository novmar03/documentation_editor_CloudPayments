...
import {documentationSettingsPath,documentationSettings,createStructureDraft,recoverStructureDraft,structureDirty,type StructureDraft} from '@/lib/structure';
...
export function StructureManager({repository,onOpen,onConnect,onChange,onClose}:{repository:Repository|null;onOpen:(id:string)=>void;onConnect:()=>void;onChange:()=>void;onClose:()=>void}){
  const [draft,setDraft]=useState<StructureDraft>(()=>createStructureDraft(navigation));
  ...
  const nodes=structureNodes(draft.groups),selected=nodes.find(n=>n.key===dialog?.key),dirty=structureDirty(draft);
  ...
  useEffect(()=>{let cancelled=false;setReady(false);setLoadError(false);void(async()=>{
    try{
      ...
      let html:string,settings;
      if(repository){const pub=publisher(),head=await pub.api('/git/ref/heads/main');const [file,config]=await Promise.all([pub.file('index.html',head.object.sha),pub.file(documentationSettingsPath,head.object.sha)]);if(!file)throw new Error();html=file.content;settings=documentationSettings(config?JSON.parse(config.content):undefined);}
      ...
      const next=recoverStructureDraft(recovered,createStructureDraft(data.groups,settings??data.settings));setDraft(next);applyNavigation(next.groups,data.pages);onChange();setReady(true);
  ...
  function changeOverview(showOverviewPage:boolean){const next={...draft,settings:{...draft.settings,showOverviewPage}};localStorage.setItem(storageKey,JSON.stringify(next));setDraft(next);onChange();}
  function loadPublished(){localStorage.removeItem(storageKey);setReload(n=>n+1);}
  ...
  function submit(){try{
    if(!dialog)return;
    if(dialog.action==='refresh'){loadPublished();setDialog(null);return;}
  ...
  async function publish(){if(!repository){onConnect();return;}setBusy(true);try{
    await publisher().publishStructure(draft.groups,draft.base,draft.settings,draft.baseSettings);
    const next=createStructureDraft(draft.groups,draft.settings);localStorage.removeItem(storageKey);setDraft(next);setDialog(null);toast.success('Структура отправлена на публикацию');
  ...
  return <section className="structure-manager" aria-label="Структура документации"><header><div><h1>Структура документации</h1><p>Порядок и вложенность не меняют адреса страниц. Структура общая для русского и английского языков.</p></div><div className="structure-actions"><Button variant="ghost" onClick={onClose}>К редактору страницы</Button><Button variant="ghost" disabled={busy} onClick={()=>dirty?setDialog({action:'refresh'}):loadPublished()}>Загрузить с сайта</Button><DropdownMenu><DropdownMenuTrigger asChild><Button disabled={!ready||busy} variant="outline"><Plus size={16}/>Добавить</Button></DropdownMenuTrigger><DropdownMenuContent><DropdownMenuItem onSelect={()=>show('page')}>Страница</DropdownMenuItem><DropdownMenuItem onSelect={()=>show('category')}>Раздел</DropdownMenuItem></DropdownMenuContent></DropdownMenu><Button disabled={!ready||busy||!dirty} onClick={()=>setDialog({action:'publish'})}>{busy?'Публикуем…':'Опубликовать структуру'}</Button></div></header>
    <div className="structure-overview"><Switch id="show-overview-page" checked={draft.settings.showOverviewPage} disabled={!ready||busy} onCheckedChange={changeOverview} aria-describedby="show-overview-help"/><div><label htmlFor="show-overview-page">Показывать страницу «Об этом документе»</label><p id="show-overview-help" className="form-help">Если выключено, документация открывается на первой доступной странице. Настройка общая для RU и EN.</p></div></div>
