...
import {navigation,pages,type NavigationPage} from '@/lib/navigation';
...
import {DOCS_URL,DOCS_ROUTING,DOCS_REPO,EDITOR_REPO} from '@/lib/config';
import {publicHeadingUrl} from '@/lib/documentation-links';
import {DocumentationLinkDialog} from './documentation-link-dialog';
import {StructureManager} from './structure-manager';
import {ImageUploadContext} from './carousel';
import {uploadEditorImage} from '@/lib/image-upload';
import {installCarouselRuntime} from '@/lib/carousel-runtime';
import {carouselScript} from '@/lib/carousel-publication';
...
const blank:DocNode={type:'doc',content:[{type:'paragraph'}]};
type Loaded={id:string;locale:Locale;title:string;html?:string;content?:DocNode;draft?:Draft;recovered?:boolean};
type SaveState='ready'|'dirty'|'saving'|'saved'|'local'|'error';
export default function EditorApp(){
  const [structureOpen,setStructureOpen]=useState(false);
  const initial=useRef(editorRoute(location.hash)).current;
  ...
  const [connectionOpen,setConnectionOpen]=useState(false),[token,setToken]=useState(''),[connecting,setConnecting]=useState(false);
  ...
  async function save(remote=true):Promise<boolean>{
    ...
    const promise=(async()=>{try{
      ...
      return true;
    }catch(e){setSaveState('error');setSaveError(errorText(e));return false;}})();
    inflight.current=promise;const success=await promise;inflight.current=null;return success;
  }
    ...
    try{
      ...
      if(!draft&&locale==='en'){
        ...
        if(repoRef.current){
          ...
          const file=await pub.file('src/content/editor-pages.en.json');
          translated=file?JSON.parse(file.content)[id]:null;
        }
        const russian=working.current?.id===id&&working.current.locale!=='en'?{...working.current,content:currentContent.current}:repoRef.current?await repoRef.current.load(id,'ru'):await recoverDraft(id,'ru').catch(()=>null);
        const content=translated?.content||russian?.content;
        // This private copy is published only when the author chooses Publish.
        draft={id,locale:'en',title:translated?.title||russian?.title||item.title,content:content?structuredClone(content):blank,updated:new Date().toISOString(),baseHtml:translated?.html||''};
  ...
  useEffect(()=>{const context=(document as any).modelContext;if(!context?.registerTool)return;const lifecycle=new AbortController();
    for(const tool of [{name:'read_document',description:'Read the current documentation page and its heading outline.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true,untrustedContentHint:true},execute:()=>({page:working.current?.id,title:working.current?.title,document:currentContent.current})},{name:'append_document_paragraph',description:'Append a paragraph to the current draft and save it using the same draft-saving flow as the editor. Does not publish the site.',inputSchema:{type:'object',properties:{text:{type:'string',maxLength:10000}},required:['text'],additionalProperties:false},annotations:{readOnlyHint:false},execute:async(input:any)=>{if(typeof input?.text!=='string'||!input.text.trim()||input.text.length>10000)throw new Error('Provide nonempty text up to 10000 characters');const ed=latestEditor.current;if(!ed)throw new Error('Page is not loaded');ed.commands.insertContentAt(ed.state.doc.content.size,{type:'paragraph',content:[{type:'text',text:input.text}]});const ok=await saveRef.current();if(!ok)throw new Error('Draft could not be saved');return {page:working.current?.id,savedTo:repoRef.current?'GitHub':'local recovery'};}}])void Promise.resolve(context.registerTool(tool,{signal:lifecycle.signal})).catch(()=>{});
    return()=>lifecycle.abort();
  },[]);
  async function connect(){
    setConnecting(true);try{
      const repo=new Repository({project:EDITOR_REPO,defaultBranch:'main',token});await repo.connect();
  ...
  function pageNavigation(items:NavigationPage[],parentId?:string):React.ReactNode {
    return items.filter(p=>!p.hidden&&p.parentId===parentId).map(p=>{
      const children=items.filter(child=>child.parentId===p.id);
      const button=p.type==='category'?<span className="page-link">{p.title}</span>:<button type="button" className={p.id===loaded?.id?'page-link active':'page-link'} aria-current={p.id===loaded?.id?'page':undefined} onClick={e=>{e.preventDefault();setStructureOpen(false);void openPage(p.id);}}>{titles[draftKey(p.id,loaded?.locale)]||p.title}</button>;
      return children.length?<details className="page-subgroup" key={p.id} open={p.id===loaded?.id||children.some(child=>child.id===loaded?.id)}><summary>{button}</summary><nav aria-label={p.title}>{pageNavigation(items,p.id)}</nav></details>:<Fragment key={p.id}>{button}</Fragment>;
    });
  }
  async function copySectionLink(id:string){
    if(!loaded)return;
    try{await navigator.clipboard.writeText(publicHeadingUrl(DOCS_URL,loaded.id,loaded.locale,id,DOCS_ROUTING));toast.success('Ссылка на раздел скопирована');}
    catch{toast.error('Не удалось скопировать ссылку. Проверьте разрешение на доступ к буферу обмена.');}
  }
  ...
  return <SidebarProvider style={{'--sidebar-width':'300px'} as CSSProperties}>
    <Sidebar className="editor-sidebar"><SidebarHeader className="brand-header"><img src="./cloudpayments-logo.svg" alt="CloudPayments"/><span>Редактор документации</span></SidebarHeader>
      <SidebarContent className="sidebar-body"><Button variant={structureOpen?'secondary':'ghost'} onClick={()=>setStructureOpen(v=>!v)}>Структура документации</Button><Tabs value={activeTab} onValueChange={setActiveTab}><TabsList className="w-full"><TabsTrigger value="pages"><FileText size={15}/>Страницы</TabsTrigger><TabsTrigger value="outline"><List size={15}/>Оглавление</TabsTrigger></TabsList>
        <TabsContent value="pages">{navigation.filter(g=>!g.hidden).map(g=>g.root?<nav key={g.id}>{pageNavigation(g.items)}</nav>:<details className="page-group" key={g.id} open={g.id===pages[loaded?.id||'tech/api']?.group}><summary>{g.title}</summary><nav>{pageNavigation(g.items)}</nav></details>)}</TabsContent>
        <TabsContent value="outline"><div className="outline-page"><FileText size={17}/><span>{loaded?.title||'API'}</span><span className="level-tag">H1</span></div>{outline.length===0?<div className="outline-empty">Добавьте заголовок — он появится здесь.</div>:<nav className="heading-tree" aria-label="Оглавление страницы">{outline.map((h,i)=>{
          ...
          return <div key={h.id}><div className={'heading-row '+(selectedHeading===h.id?'selected':'')} style={{paddingLeft:8+(h.level-2)*12}}>{children?<button type="button" className="disclosure" aria-label={collapsed.has(h.id)?'Раскрыть раздел':'Свернуть раздел'} aria-expanded={!collapsed.has(h.id)} onClick={()=>setCollapsed(old=>{const s=new Set(old);s.has(h.id)?s.delete(h.id):s.add(h.id);return s;})}>{collapsed.has(h.id)?<ChevronRight size={14}/>:<ChevronDown size={14}/>}</button>:<span className="outline-dot"/>}<button type="button" className="heading-name" onClick={()=>jump(h.id)}>{h.title||'Без названия'}</button><button type="button" className="heading-copy-link" aria-label={'Скопировать ссылку на раздел: '+(h.title||'Без названия')} title="Скопировать ссылку на раздел" onClick={()=>void copySectionLink(h.id)}><Link2 size={14}/></button></div>{!preview&&selectedHeading===h.id&&<div className="heading-controls"><Choice label="Уровень заголовка" value={String(h.level)} onChange={v=>editor!.chain().focus().setHeading({level:Number(v) as any}).run()} options={[2,3,4,5,6].map(n=>[String(n),'H'+n])}/><IconButton label="Переместить раздел вверх" onClick={()=>shiftHeading(h.index,-1)}><ArrowUp size={16}/></IconButton><IconButton label="Переместить раздел вниз" onClick={()=>shiftHeading(h.index,1)}><ArrowDown size={16}/></IconButton></div>}</div>;
    ...
    <Sheet open={settingsOpen} onOpenChange={setSettingsOpen}><SheetContent><SheetHeader><SheetTitle>Настройки блока</SheetTitle></SheetHeader>{pageSettings}<div className="p-5">{properties}</div></SheetContent></Sheet>
...
function DocumentWorkspace({loaded,preview,onEditor,onSelection,onReady,onChange,onTitle,onConnect,repository}:{loaded:Loaded;preview:boolean;onEditor:(ed:Editor)=>void;onSelection:()=>void;onReady:(doc:DocNode)=>void;onChange:(doc:DocNode)=>void;onTitle:(t:string)=>void;onConnect:()=>void;repository:Repository|null}){
  const initializing=useRef(true),onChangeRef=useRef(onChange),onSelectionRef=useRef(onSelection);onChangeRef.current=onChange;onSelectionRef.current=onSelection;
  const editor=useEditor({extensions:extensions(),content:loaded.content||loaded.html||blank,immediatelyRender:true,editorProps:{attributes:{class:'document-editor',spellcheck:'true','aria-label':'Содержание страницы'}},onCreate:({editor:ed})=>{const doc=normalizeHeadings(ed.getJSON() as DocNode);ed.commands.setContent(doc,{emitUpdate:false});initializing.current=false;onReady(doc);onEditor(ed);},onUpdate:({editor:ed})=>{if(!initializing.current)onChangeRef.current(ed.getJSON() as DocNode);},onSelectionUpdate:()=>onSelectionRef.current()});
  const [linkOpen,setLinkOpen]=useState(false),previewRoot=useRef<HTMLDivElement>(null);
  useEffect(()=>{if(preview)return installCarouselRuntime(previewRoot.current);},[preview,editor]);
  ...
  return <><div className="document-heading"><div className="page-eyebrow">{navigation.find(g=>g.id===pages[loaded.id].group)?.title}</div>{preview?<h1>{loaded.title}</h1>:<input className="page-title-input" aria-label="Название страницы H1" value={loaded.title} onChange={e=>onTitle(e.target.value)} placeholder="Название страницы"/>}</div>{!preview&&<div className="format-toolbar"><div className="format-type"><Choice label="Тип текста" value={paragraph} onChange={value=>value==='paragraph'?editor.chain().focus().setParagraph().run():editor.chain().focus().setHeading({level:Number(value.slice(1)) as any}).run()} options={[['paragraph','Обычный текст'],...[2,3,4,5,6].map(n=>['h'+n,'Заголовок H'+n])]}/></div><span className="toolbar-divider"/><IconButton label="Жирный" active={editor.isActive('bold')} onClick={()=>editor.chain().focus().toggleBold().run()}><Bold size={17}/></IconButton><IconButton label="Курсив" active={editor.isActive('italic')} onClick={()=>editor.chain().focus().toggleItalic().run()}><Italic size={17}/></IconButton><IconButton label="Подчёркнутый" active={editor.isActive('underline')} onClick={()=>editor.chain().focus().toggleUnderline().run()}><Underline size={17}/></IconButton><IconButton label="Ссылка" active={editor.isActive('link')} onClick={()=>setLinkOpen(true)}><Link2 size={17}/></IconButton><span className="toolbar-divider"/>{[['left',AlignLeft,'По левому краю'],['center',AlignCenter,'По центру'],['right',AlignRight,'По правому краю']].map(([value,Icon,label])=><IconButton key={value as string} label={label as string} active={editor.isActive({textAlign:value})} onClick={()=>editor.chain().focus().setTextAlign(value as string).run()}>{typeof Icon!=='string'&&<Icon size={17}/>}</IconButton>)}<IconButton label="Маркированный список" active={editor.isActive('bulletList')} onClick={()=>editor.chain().focus().toggleBulletList().run()}><List size={17}/></IconButton><IconButton label="Нумерованный список" active={editor.isActive('orderedList')} onClick={()=>editor.chain().focus().toggleOrderedList().run()}><ListOrdered size={17}/></IconButton><span className="toolbar-divider"/><IconButton label="Отменить" disabled={!editor.can().undo()} onClick={()=>editor.chain().focus().undo().run()}><Undo2 size={17}/></IconButton><IconButton label="Повторить" disabled={!editor.can().redo()} onClick={()=>editor.chain().focus().redo().run()}><Redo2 size={17}/></IconButton><InsertMenu editor={editor}/></div>}
    ...
    <DocumentationLinkDialog editor={editor} open={linkOpen} onOpenChange={setLinkOpen} pageId={loaded.id} locale={loaded.locale} repository={repository}/>
  </>;
}
function InsertMenu({editor,bottom=false}:{editor:Editor;bottom?:boolean}){
  const items=[['text','Текст',Type],['heading','Заголовок',Heading],['code','Код',Code2],['info','Информация',Info],['attention','Внимание',TriangleAlert],['table','Таблица',Table2],['image','Фото / GIF',ImagePlus],['carousel','Карусель',ImagePlus],['button','Кнопка со ссылкой',MousePointer2]] as const;
  function insert(type:string){
    let node:any={type:'paragraph'};
    if(type==='carousel')node={type:'carousel',attrs:{id:crypto.randomUUID()},content:[]};
    ...
    if(type==='button')node={type:'docButton',attrs:{label:'Подробнее',href:'https://cloudpayments.ru/',width:240,height:56,fontSize:18,align:'center'}};
    ...
    const chain=editor.chain().focus().insertContentAt(pos,node);
    if(type==='carousel')chain.setNodeSelection(pos).run();else chain.setTextSelection(pos+1).run();
  }
...
function BlockSettings({editor,tick,repository,onConnect}:{editor:Editor;tick:number;repository:Repository|null;onConnect:()=>void}){
  const fileRef=useRef<HTMLInputElement>(null),[uploading,setUploading]=useState(false),[remove,setRemove]=useState(false);
  const [imageDialog,setImageDialog]=useState(false),[imageUrl,setImageUrl]=useState('');
  const {$from,node}=editor.state.selection as any;
  const selectedType=node?.type.name||($from.parent.type.name==='heading'?'heading':editor.isActive('codeBlock')?'codeBlock':editor.isActive('callout')?'callout':editor.isActive('table')?'table':$from.parent.type.name);
  ...
  return <div className="block-settings"><div className="block-type-label">{title}</div><input ref={fileRef} type="file" accept="image/png,image/jpeg,image/webp,image/gif" hidden onChange={e=>void upload(e.target.files?.[0])}/>
    ...
    <Button className="delete-block" variant="ghost" onClick={()=>setRemove(true)}><Trash2 size={16}/>Удалить блок</Button>
    <AlertDialog open={remove} onOpenChange={setRemove}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Удалить выбранный блок?</AlertDialogTitle><AlertDialogDescription>Действие можно отменить кнопкой «Отменить» в панели редактора.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Отмена</AlertDialogCancel><AlertDialogAction onClick={()=>{if(editor.isActive('table'))editor.chain().focus().deleteTable().run();else if(node)editor.chain().focus().deleteSelection().run();else {const from=editor.state.selection.$from;editor.chain().focus().deleteRange({from:from.before(1),to:from.after(1)}).run();}}}>Удалить</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
    <Dialog open={imageDialog} onOpenChange={setImageDialog}><DialogContent><DialogHeader><DialogTitle>Добавить изображение</DialogTitle><DialogDescription>Укажите прямую ссылку на изображение или GIF.</DialogDescription></DialogHeader><Input aria-label="Ссылка на изображение" placeholder="https://…/image.gif" value={imageUrl} onChange={e=>setImageUrl(e.target.value)}/><DialogFooter><Button onClick={()=>{const src=safeLink(imageUrl,true);if(!src||!src.startsWith('http')){toast.error('Укажите ссылку https:// на изображение');return;}editor.chain().focus().setImage({src}).run();setImageDialog(false);setImageUrl('');}}>Добавить</Button></DialogFooter></DialogContent></Dialog>
  </div>;
}
