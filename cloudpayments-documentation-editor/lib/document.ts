...
export function renderDocument(doc:DocNode):string {
  ...
  const render=(n:DocNode):string=>{
    ...
    switch(n.type){
      ...
      case 'callout':return `<aside class="api-callout ${a.kind==='attention'?'warning':''}" role="note">${inside()}</aside>`;
      case 'table':return `<div class="api-table-scroll" tabindex="0" aria-label="Таблица"><table>${inside()}</table></div>`;
